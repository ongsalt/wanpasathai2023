...
function addPointerSound(element, eventName, speedFunc, maxTimeBetweenImpacts = MAX_TIME_BETWEEN_IMPACTS) {
    ...
    element.addEventListener(eventName, (event) => {
        ...
        //     startTime = Date.now()
        // }
        // const rate = counter / ((Date.now() - startTime) / 1000)
        const pointerSpeedN = speedFunc(event);
        console.log(pointerSpeedN)
...
/* 
    TODO
    use addPointerSound as base then

    Connection
        Serial event:
            - 1 ESP for gyroscope one for decoding and transfering data to PC serial port
            - Format max at ten digit long char
            - 9600 BPS for HC-05
    
        - What if I fucking use Rust

    Change impact sound algorithm:
        - Check acceleration in the moment of colission 
        - need threshold test
*/

function addExternalAccelerometerSound() {
